...
注意: エージェントオーケストレーション実行中は、`.github/**` を対象にした Git 操作を禁止します。以下は人手での運用時のみ実施してください。
...
- ただし、`.github/**` 変更はエージェントの Git 操作対象外とし、手動 Git 対応のフォローアップとして分離する（この理由だけでタスクを `blocked` にしない）。
- エージェント実行時は `.github/**` を対象にした Git コマンド（例: `git add .github`, `git restore .github`, `git checkout -- .github`）を禁止する。
